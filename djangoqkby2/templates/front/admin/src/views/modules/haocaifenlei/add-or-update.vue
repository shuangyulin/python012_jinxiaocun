<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="耗材分类" prop="haocaifenlei">
          <el-input v-model="ruleForm.haocaifenlei" 
              placeholder="耗材分类" clearable  :readonly="ro.haocaifenlei"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="耗材分类" prop="haocaifenlei">
              <el-input v-model="ruleForm.haocaifenlei" 
                placeholder="耗材分类" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='haocaifenlei'){
            this.ruleForm.haocaifenlei = obj[o];
	    this.ro.haocaifenlei = true;
            continue;
          }
        }
      }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `haocaifenlei/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.haocaifenleiCrossAddOrUpdateFlag = false;
						 this.parent.search();
